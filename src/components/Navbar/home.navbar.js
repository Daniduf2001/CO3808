...
const HomeNavbar = () => {
    ...
    return (
        <nav className='bg-dark-blue text-light topnav'>
            <div id="sidenav" className="sidenav">
                <span className="closebtn nav-ham" onClick={CloseNav}>&times;</span>
                <NavLink to="/adminTeacher" className="sidenav-title">Dashboard</NavLink>
                <NavLink to="/" className="sidenav-title">Classes</NavLink>
                {classes.map(_class => {
                    if (!_class.archived && !userInfo.archived_class.includes(_class._id)) {
                        return <NavLink to={`/class/${_class._id}`} key={_class._id}
                                        className="sidenav-item">{_class.title}</NavLink>
                    } else return null;
                })}
            ...
            {Profile === null
                ? <img src={DefaultProfile} alt="Default Profile Logo" className="nav-right pp nav-ham"
                       onClick={() => window.location = "/profile"}></img>
                : <img src={Profile} alt="Profile Logo" className="nav-right pp nav-ham"
                       onClick={() => window.location = "/profile"}></img>
            }
            <span className="nav-right nav-ham" style={{marginTop: "3px"}}
                  onClick={() => window.location = "/class/join"}>+</span>
        </nav>
    )
}
