                    ...
                    <div className="container" style={{marginTop: '100px'}}>
                        <form className="margin-top-bottom box box-shadow" onSubmit={updateClassInfo}>
                            ...
                            <div className="form-group">
                                ...
                                <input className="form-control" type="text" value={inputTitle}
                                       onChange={({target: {value}}) => setInputTitle(value)}/>
                            </div>
                            <div className="form-group">
                                <p className="form-label">Description:</p>
                                <textarea rows="5" className='form-control' value={inputDescription}
                                          onChange={({target: {value}}) => setInputDescription(value)}></textarea>
                            </div>
                            <div className="form-group">
                                <input type="submit" className="form-control btn btn-dark-blue" value="update"/>
                            </div>
                        </form>
                                ...
                                {teachers.length > 0 ?
                                    teachers.map(teacher => {
                                        return <li key={teacher._id}>{teacher.username} ({teacher.email})
                                            <ul>
                                                <li><p className="link" onClick={() => toStudent(teacher._id)}>Demote
                                                    become a
                                                    student</p></li>
                                                <li><p className="link"
                                                       onClick={() => removeTeacher(teacher._id)}>Remove</p>
                                                </li>
                                            </ul></li>
                                    })
                                ...
                                {students.length > 0 ?
                                    students.map(student => {
                                        return <li key={student._id}>{student.username} ({student.email})
                                            <ul style={{listStyle: 'none'}}>
                                                <li><p className="btn link text-success"
                                                       onClick={() => toTeacher(student._id)}>Promote
                                                    become a
                                                    teacher</p></li>
                                                <li><p className="btn link text-danger"
                                                       onClick={() => removeStudent(student._id)}>Remove</p>
                                                </li>
                                            </ul></li>
                                    })
                                    : <h3>There is no students yet. Send class code({classInfo.code}) to your
                                        student.</h3>}
                            </ul>
                        </div>
                        <form className="margin-top-bottom box box-shadow" onSubmit={ArchiveClass}>
                            ...
                            <div className="form-group">
                                <input type="submit" className="form-control btn btn-dark-blue" value="Archive"/>
                            </div>
                        </form>
                    </div>
                </div>
