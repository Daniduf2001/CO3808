...
const ShortAnswer = (params) => {
        ...
        Axios.get(`${URL}/classwork/get/${classworkId}`)
            .then(res => {
                if (res.data.types === "short answer") {
                    ...
                    navigate.push('/teacherAdmin/class/', {
                        state: {
                            classID: classID
                        }
                    })
    ...
    const updateClasswork = e => {
        ...
        Axios.post(`${URL}/classwork/update/${classwork._id}`, {
            title: inputTitle, description: inputDescription, duedate: inputDeadline, token
        })
            .then(res => {
    ...
    const Answer = e => {
        ...
        if (!answered) {
            Axios.post(`${URL}/classwork/submit/answer`, {
                token, classwork: classwork._id, answer: inputAnswer, student: userInfo._id
            })
        ...
        <div className="main_container">
            <div className="item">
                <div className="container-fluid">
                    ...
                    <div className="container margin-top-bottom" style={{width: "95%"}}>
                        <div className="col-9">
                            ...
                            <p>posted {moment(classwork.createdAt).fromNow()}
                                {classwork.createdAt !== classwork.updatedAt ?
                                    <span>(updated {moment(classwork.updatedAt).fromNow()})</span> : null} by {author}</p>
                            {classwork.author === userInfo._id ?
                                <div><h3><span style={{paddingLeft: "0"}} className="link"
                                               onClick={openClasswork}>Edit</span></h3>
                                    <h3><span style={{paddingLeft: "0"}} className="link text-danger"
                                              onClick={deleteClasswork}>Delete</span></h3>
                                </div> : null}
                        </div>
                        {classInfo.teacher && !(classInfo.teacher.includes(userInfo._id) || classInfo.owner === userInfo._id) ?
                            <div className="col-3">
                                {!answered ? <form className="box box-shadow" onSubmit={Answer}>
                                    <h1 className="box-title">Your answer:</h1>
                                    <div className="form-group">
                                        <input type="text" className="form-control" value={inputAnswer}
                                               onChange={({target: {value}}) => setInputAnswer(value)}/>
                                    </div>
                                    <div className="form-group">
                                        <input type="submit" className="form-control btn btn-dark"/>
                                    </div>
                                </form> : <div className="box box-shadow">
                                ...
                                <div className="box box-shadow">
                                    ...
                                    <button className="form-control btn btn-dark" onClick={showAnswer}>See answers
                                    </button>
                                </div>
                            </div>}
                    </div>
                    {classwork.author === userInfo._id ? <div className="classwork-modal" id="classwork">
                        <div className="classwork-content container" style={{width: "50%", marginTop: "80px"}}>
                    ...
                    {classInfo.teacher && classwork.author && (classwork.author === userInfo._id || classInfo.teacher.includes(userInfo._id) || classInfo.owner === userInfo._id) ?
                        <div className="classwork-modal" id="answers">
                            <div className="classwork-content container" style={{width: "50%", marginTop: "80px"}}>
                                ...
                                {answers.map(answer => {
                                    return <p
                                        key={answer._id}>{answer.student.username} answered <b>{answer.answer}</b> {moment(answer.answeredOn).fromNow()}
                                        {answer.answeredOn > classwork.duedate ?
                                            <span><b> (Turned in late)</b></span> : null}
                                    </p>
                                })}
                            </div>
                        </div> : null}
